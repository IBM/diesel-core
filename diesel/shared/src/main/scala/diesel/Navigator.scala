...
//        ambiguity match {
//          case Some(a) => a.abort(backPtrs.size - subtrees.size)
//          case None    =>
//        }
//      }
//    }
...
class Navigator(
  ...
  private val root: StateIterator = new StateIterator(result.successState, this)
  ...
  private[diesel] class SentinelIterator(navigator: Navigator)
      extends Iterator[Seq[Parsing]] {

    private var item: Option[Seq[Parsing]] = Some(Seq())

    override def next(): Seq[Parsing] = {
      item match {
        case Some(value) => item = None; value
  ...
  private[diesel] class ItemIterator(parsing: Parsing, navigator: Navigator)
      extends Iterator[Seq[Parsing]] {

    private var item: Option[Parsing] = Some(parsing)

    override def next(): Seq[Parsing] = {
      item match {
        case Some(value) => item = None; Seq(value)
        case None        =>
  ...
  private[diesel] class BackPtrIterator(
    ...
    private var predIterator = new StateIterator(predecessor, navigator)
    ...
    override def next(): Seq[Parsing] = {
      ...
      if (!predIterator.hasNext) {
        if (causal.hasNext) {
          current = causal.next()
          predIterator = new StateIterator(predecessor, navigator)
          finished = true
        }
      }
      result
  ...
  private class StateIterator(val state: State, navigator: Navigator)
      extends Iterator[Seq[Parsing]] {
    ...
    private def moveToNext(first: Boolean): Option[Iterator[Seq[Parsing]]] = {
      val res = if (index.isEmpty) {
        if (first) Some(new SentinelIterator(navigator)) else None
      } else {
        val backPtr = index.head
        index = index.tail
        val causal  = backPtr.causal match {
          case item: TerminalItem => new ItemIterator(applyToken(item), navigator)
          case state: State       => new StateIterator(state, navigator)
        }
        Some(new BackPtrIterator(
          causal,
          backPtr.predecessor,
          navigator
        ))
      ...
      if (state.isCompleted) {
        res match {
          case Some(value) =>
            val subtrees = collectSubtrees(value)
    ...
    private def collectSubtrees(value: Iterator[Seq[Parsing]]): Seq[Seq[Parsing]] = {
      var subtrees: Seq[Seq[Parsing]] = Seq()
      while (value.hasNext) {
        val subtree: Seq[Parsing] = value.next()
        if (
